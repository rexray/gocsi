...
// LogCreateVolume is a gRPC interceptor that logs the arguments for
// the eponymous message.
func LogCreateVolume(
	ctx context.Context,
	req interface{},
	info *grpc.UnaryServerInfo,
	handler grpc.UnaryHandler) (resp interface{}, err error) {

	treq, ok := req.(*csi.CreateVolumeRequest)
	if !ok {
		return handler(ctx, req)
	}

	louti.Printf(
		"CreateVolume.Version=%s\n",
		gocsi.SprintfVersion(treq.GetVersion()))
	louti.Printf(
		"CreateVolume.CapacityRange=%+v\n",
		treq.GetCapacityRange())
	louti.Printf(
		"CreateVolume.Name=%v\n",
		treq.GetName())
	louti.Printf(
		"CreateVolume.Parameters=%+v\n",
		treq.GetParameters())
	louti.Printf(
		"CreateVolume.VolumeCapabilities=%+v\n",
		treq.GetVolumeCapabilities())
...
func (s *sp) ControllerPublishVolume(
	...
	*csi.ControllerPublishVolumeResponse, error) {
	...
	louti.Printf(
		...
		gocsi.SprintfVersion(req.GetVersion()))
	louti.Printf(
	...
	louti.Printf("mock.ControllerPublishVolums.Response=%+v\n", resp)
	return resp, nil
}
...
// LogListVolumes is a gRPC interceptor that logs the arguments for
// the eponymous message.
func LogListVolumes(
	ctx context.Context,
	req interface{},
	...
	treq, ok := req.(*csi.ListVolumesRequest)
	if !ok {
		return handler(ctx, req)
	}

	louti.Printf(
		"ListVolumes.Version=%s\n",
		gocsi.SprintfVersion(treq.GetVersion()))
	louti.Printf(
		"ListVolumes.MaxEntries=%d\n",
		treq.GetMaxEntries())
	louti.Printf(
		"ListVolumes.StartingToken=%s\n",
		treq.GetStartingToken())
